      ...
      IF (IRT .EQ. 1) THEN
         CALL WARNR (IW1, 'AMU < ALAM in ALPI', 'MU', AMU,
     >              ALM, BIG, 1)
      ELSEIF (IRT .EQ. 2) THEN
         CALL WARNR (IW2, 'ALPI > 3; Be aware!', 'ALPI', ALPI,
     >             D0, D1, 0)
      ENDIF
      RETURN
      END
     ...
     >  'Order parameter out of range in ALPQCD; IRDR = ', IRDR
      ...
      IF     (TEM .NE. D0) THEN
        TMP = D1 / TEM
      Else
      CALL WARNR(IWRN, 'DXDZ is singular in DXDZ; DXDZ set=HUGE',
     >             'Z', Z, D0, D1, 0)
        TMP = HUGE
      EndIf
      DXDZ = TMP
...
669	   format(1x,'evolve.f:  nx,nt,jt,ng=',4i4,
	...
	enddo
      ...
      FINTRP = 0.
      IF (NX .LT. 1) THEN
         CALL WARNI(IW1, 'Nx < 1, error in FINTRP.',
     >              'NX', NX, 1, 256, 1)
         IR = 1
         RETURN
      ELSE
      ...
      ENDIF
      IF (DX .LE. 0) THEN
         CALL WARNR(IW3, 'DX < 0, error in FINTRP.',
     >              'DX', DX, D0, D1, 1)
         IR = 2
         RETURN
      ENDIF
...
50      FORMAT (' GausInt FAILS, GausInt,NMAX,XL,XR=',G15.7,I5,2G15.7)
  ...
  230 CONTINUE
      ...
      IF (NX .LT. 1 .OR. XA(NX-1,1) .EQ. 0D0) THEN
        ...
        CALL WARNI (IWRN1, MSG, 'NX', NX, 0, MXX, 0)
        IRR = 1
      EndIf
      IF (M .LT. M1 .OR. M .GT. M2) THEN
        MSG ='Exponent M out of range in INTEGR'
        CALL WARNI (IWRN2, MSG, 'M', M, M1, M2, 1)
        IRR = 2
      EndIf
      G(NX) = 0D0
   ...
   10 CONTINUE
     ...
     >(XX, FF1, FG1, GF1, GG1, PNSP, PNSM, FF2, FG2, GF2, GG2, NFL, IRT)
      ...
      IF (X .LE. 0. .OR. X .GE. 1.) THEN
        CALL WARNR(IWRN, 'X out of range in KERNEL', 'X', X,
     >             D0, D1, 1)
        IRT = 1
        RETURN
      EndIf
...
20    RETURN
 ...
 11   If (JU-JLx .GT. 1) Then
      ...
      ElseIf (JLx .Eq. 0) Then
         ...
         CALL WARNR (IWRN1, Msg, 'X', X, Xmin, 1D0, 1)
 ...
 12   If (JU-JLq .GT. 1) Then
      ...
      If     (JLq .LE. 0) Then
         ...
         If (JLq .LT. 0) Then
          ...
          CALL WARNR (IWRN2, Msg, 'Q', Q, Qini, 1D0, 1)
         EndIf
      ...
      Else
        ...
        If (JLq .GE. Nt) Then
         ...
         CALL WARNR (IWRN3, Msg, 'Q', Q, Qmax, 1D0, 1)
        Endif
      Endif
      tt = log(log(q/al))
      ...
      Do it = 1, nqvec
       ...
       If (Jx .Eq. 0) Then
         ...
         Call POLIN1 (XVpow(0), Fij(1), 4, ss, Fx, Dfx) 
         
         If (x .GT. 0D0)  Fvec(it) =  Fx / x**2 
       ElseIf  (JLx .Eq. Nx-1) Then
        Call POLIN1 (XVpow(Nx-3), Upd(J1), 4, ss, Fx, Dfx)
        Fvec(it) = Fx
       Else 
         sf2 = Upd(J1+1)
         sf3 = Upd(J1+2)
     ...
     &               + sf2*sy3 - sf3*sy2) / s23
      ...
      enddo
      If (JLq .LE. 0) Then
        Call POLIN1 (TV(0), Fvec(1), 4, tt, ff, Dfq)
      ElseIf (JLq .GE. Nt-1) Then
        Call POLIN1 (TV(Nt-3), Fvec(1), 4, tt, ff, Dfq)
      Else
        tf2 = fvec(2)
        tf3 = fvec(3)
     ...
     &	  +  (tmp1*ty2-t12*ty3)*(fvec(4)-g4)/t34)
      ...
      EndIf
      PARDIS = ff
c enforce threshold exactly...
	ii = iabs(IPRTN)
	if(ii .ne. 0) then
	   if(QQ .le. VALQMS(ii) ) pardis = 0.d0
	endif
      Return
      End
   ...
   15 CONTINUE
	...
	PRINT *, 'stop (former pause) in parpdf'
	stop
      EndIf
      IRET= JRET
      RETURN
  100 FORMAT (/)
    ...
    2 IF (LA .AND. .NOT.LB) THEN
      ...
      Else
        CALL POLIN1 (XL, FF1(1), MX, X, TEM, ERR)
      EndIf
      IF (LA) THEN
         IF (LB) THEN
            PFF1 = TEM / (1.-X)
      ...
      Else
        CALL POLIN1 (XL, PNSP(1), MX, X, TEM, ERR)
      EndIf
      FNSP = TEM / (1.- X)
      RETURN
      ENTRY FNSM (XX)
      ...
      Else
        CALL POLIN1 (XL, PNSM(1), MX, X, TEM, ERR)
      EndIf
      FNSM = TEM / (1.- X)
      RETURN
      ENTRY RGG1 (XX)
      ...
      Else
        CALL POLIN1 (XL, GG1(1), MX, X, TEM, ERR)
      EndIf
      IF (LA) THEN
         PGG1 = TEM / X / (1.-X)
         LA   =.FALSE.
      ...
      Else
        CALL POLIN1 (XL, FF2(1), MX, X, TEM, ERR)
      EndIf
      IF (LA) THEN
         PFF2 = TEM / X / (1.-X)
         LA   =.FALSE.
      ...
      Else
        CALL POLIN1 (XL, GG2(1), MX, X, TEM, ERR)
      EndIf
      IF (LA) THEN
         PGG2 = TEM / X / (1.-X)
         LA   =.FALSE.
...
11    CONTINUE
      ...
      DO 13 M=1,N-1
        DO 12 I=1,N-M
          ...
          IF(DEN.EQ.0.)then
             write(6,50)
50           format(1x,'fatal error (former PAUSE): DEN=0 in POLIN1')
             stop
          ENDIF
          DEN=W/DEN
          D(I)=HP*DEN
...
11    CONTINUE
      ...
      DO 13 M=1,N-1
        DO 12 I=1,N-M
          ...
          IF(DD.EQ.0.)then
	     write(6,50)
50	     format(1x,'fatal error (former PAUSE): DD=0 in ratint')
	     stop
	  ENDIF
          DD=W/DD
          D(I)=C(I+1)*DD
...
13    CONTINUE
   ...
   60    CONTINUE
	...
	stop
      ...
      IF (DX .LE. 0.) THEN
        CALL WARNR(IW2,'DX cannot be < 0. in SMPSNA', 'DX', DX,
     >               D0, D1, 0)
        SMPSNA = 0.
        RETURN
      ENDIF
      IF (NX .LE. 0 .OR. NX .GT. MAXX) THEN
        CALL WARNI(IW1, 'NX out of range in SMPSNA', 'NX', NX,
     >               1, MAXX, 1)
        SIMP = 0.
      ELSEIF (NX .EQ. 1) THEN
        SIMP = 0.
  ...
  240 CONTINUE
   ...
   21 CONTINUE
     ...
     >                  PNSP(0:MXX), PNSM(0:MXX)
      ...
      COMMON / KRNL01 / AFF2 (MXX),AFG2 (MXX), AGF2 (MXX), AGG2 (MXX),
     >                  ANSP (MXX), ANSM (MXX), ZFG2, ZGF2, ZQQB
      EXTERNAL PFF1, RGG1, RFF2, RGG2
      EXTERNAL FNSP, FNSM
      dimension aff1(mxx),agg1(mxx)
      PARAMETER (PI = 3.141592653589793d0, PI2 = PI ** 2)
      DATA CF, CG, TR, IWRN / 1.33333333333333d0, 3.0, 0.5, 0 /
   ...
   10 CONTINUE
      ...
      GG2(NX) = GG2(NX-1) * 3. - GG2(NX-2) * 3. + GG2(NX-3)
         RER = RERR * 4.
         AFF1 (1) = GausInt(PFF1, D0, XV(1), AERR, RERR, ER1, IRT)
         DGG1     = NFL / 3.
         TMPG     = GausInt(RGG1, D0, XV(1), AERR, RERR, ER3, IRT)
         AGG1 (1) = TMPG + DGG1
       ANSM (1) = GausInt(FNSM, D0, XV(1), AERR, RER, ER2, IRT)
       ANSP (1) = GausInt(FNSP, D0, XV(1), AERR, RER, ER2, IRT)
         AER = AFF1(1) * RER
         AFF2 (1) = GausInt(RFF2, D0, XV(1),  AER, RER, ER2, IRT)
         AER = AGG1(1) * RER
         AGG2 (1) = GausInt(RGG2, D0, XV(1),  AER, RER, ER4, IRT)
      DO 20 I2 = 2, NX-1
      TEM =GausInt(PFF1,XV(I2-1),XV(I2),AERR,RERR,ER1,IRT)
      AFF1(I2) = TEM + AFF1(I2-1)
      AER = ABS(TEM * RER)
      AFF2(I2)=GausInt(RFF2,XV(I2-1),XV(I2),AER,RER,ER2,IRT)
     >        +AFF2(I2-1)
      TEM      = GausInt(RGG1,XV(I2-1),XV(I2),AERR,RERR,ER3,IRT)
      TMPG     = TMPG + TEM
      AGG1(I2) = TMPG + DGG1
      AER = ABS(TEM * RER)
      AGG2(I2)=GausInt(RGG2,XV(I2-1),XV(I2),AER,RER,ER4,IRT)
     >        +AGG2(I2-1)
      ANSP(I2)=GausInt(FNSP,XV(I2-1),XV(I2),AERR,RER,ER4,IRT)
     >        +ANSP(I2-1)
      ANSM(I2)=GausInt(FNSM,XV(I2-1),XV(I2),AERR,RER,ER4,IRT)
     >        +ANSM(I2-1)
   20 CONTINUE
      ANSP(NX)=GausInt(FNSP,XV(NX-1),D1,AERR,RER,ERR, IRT) +ANSP(NX-1)
      ANSM(NX)=GausInt(FNSM,XV(NX-1),D1,AERR,RER,ERR, IRT) +ANSM(NX-1)
           TRNF = TR * NFL
 ...
 1    CONTINUE
      ...
      RETURN
      END
      SUBROUTINE WARNI (IWRN, MSG, NMVAR, IVAB,
     >                  IMIN, IMAX, IACT)
      CHARACTER*(*) MSG, NMVAR
      Save Iw
      Data Nmax / 100 /
      ...
      Elseif (Iw .Eq. Nmax) Then
         Print '(/A/)', 'WARNI Severe Warning: Too many errors'
      Endif
      IWRN = IW + 1
      RETURN
      END
      SUBROUTINE WARNR (IWRN, MSG, NMVAR, VARIAB,
     >                  VMIN, VMAX, IACT)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
      PARAMETER (D0=0D0, D1=1D0, D2=2D0, D3=3D0, D4=4D0, D10=1D1)
      CHARACTER*(*) MSG, NMVAR
     ...
     >                  NMVAR, ' = ', VR
      Elseif (Iw .Eq. Nmax) Then
         Print '(/A/)', 'WARNR Severe Warning: Too many errors'
      Endif
      IWRN = IW + 1
      RETURN
      END
   ...
   17 CONTINUE
     ...
     >      - F31*F22*F13 - F21*F12*F33 - F11*F32*F23
        IF (ABS(DET) .LT. PUNY) THEN
           ...
           CALL WARNR(IWRN, Msg, 'DET', PUNY, D0, D0, 0)
           DET = PUNY
        EndIf
   ...
   30 CONTINUE
    ...
    7 EPS = TEM * RER
      ...
      Else
        CALL WARNR (IWRN2, 'Z out of range in XFRMZ, X set=0.',
     >              'Z', Z, ZLOW, ZHIGH, 1)
        TEM = 0
      EndIf
      XFRMZ = EXP(TEM)
      ...
      implicit NONE
      integer NCUT, i,n,m3
      real*8 xLi,Out,x,pi2by6,zeta3,c1,c2
      real*8 r,xt,L,xln1m
      parameter (m3=8)
...
11    CONTINUE
      ...
      ElseIF (X .GE. D0) THEN
         ...
         TEM = F(X) / F(D1)
      Else
         CALL WARNR(IWRN1, 'X out of range in ZFRMX, Z set to 99.'
     >             , 'X', X, TINY, HUGE, 1)
         TEM = 99.
         STOP
      EndIf
      ...
      ElseIF (X .GE. D0) THEN
         ...
         TEM = D(X) / F(D1)
      Else
         CALL WARNR(IWRN1, 'X out of range in DZDX, Z set to 99.'
     >             , 'X', X, TINY, HUGE, 1)
         TEM = 99.
         STOP
      EndIf
