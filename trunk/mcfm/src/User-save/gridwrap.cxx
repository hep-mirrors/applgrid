...
extern "C" void book_grid__()  // inital grid booking
{
  ...
  std::cout<<" ***********************************************"<<std::endl;
  std::cout<<"Booking the grids = "<<std::endl;
  ...
  for(int igrid=0; igrid < Ngrids; igrid++)
    {
      ...
      if ( ! file_exists(gridFiles[igrid]) ) { 
	...
	mygrid[igrid] = new appl::grid( nObsBins[igrid], obsBins[igrid], // obs bins
					...
					nXbins,   xLow,  xUp, 5,         // x bins and interpolation order
					"mcfm-w" );
	...
	mygrid[igrid] = new appl::grid(gridFiles[igrid]); //optimierte grid x,Q2 bins
  ...
  for(int igrid = 0; igrid < Ngrids; igrid++)
    {
      ...
      mygrid[igrid]->Write(gridFiles[igrid]);
      ...
      {
	// Normalise the referenece histogram
