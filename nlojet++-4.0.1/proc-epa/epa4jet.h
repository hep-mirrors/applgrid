...
#ifndef __NLO_EPA4JET_H__
#define __NLO_EPA4JET_H__ 1
...
namespace nlo {
  ...
  public:
    //   constructor
    explicit epa4jet(const random_generator&, bool=false, unsigned int=2, unsigned int=3, double=1.0);
  ...
  private:
    ...
    //  dipole contributions
    typedef double (epa4jet::*dipole_func)(int, int);
    static dipole_func _S_dipole[10];
    
    //       dipole functions
    double _M_d12(int, int);
    double _M_d13(int, int);
    double _M_d14(int, int);
    double _M_d15(int, int);
    double _M_d23(int, int);
    double _M_d34(int, int);
    double _M_d35(int, int);
  };
}  //  namespace nlo
