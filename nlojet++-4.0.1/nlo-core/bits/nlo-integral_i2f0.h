...
#ifndef __NLO_NLO_INTEGRAL_I2F0_H__
#define __NLO_NLO_INTEGRAL_I2F0_H__ 1
...
namespace nlo {
  ...
  class integral<jetfunc<amplitude<process<_Weight, _Event, _EvenTraits, 2U, 0U> > > >
  {
  public:
    //    types
  ...
  template<typename _Weight, class _Event, class _EvenTraits>
  void integral<jetfunc<amplitude<process<_Weight, _Event, _EvenTraits, 2U, 0U> > > >::
  ...
  calculate_nlo(unsigned long mxne, unsigned int time_rate_rf) 
  {
	...
	//----- finite terms -----
	eta = (p1[-1]*p1[0])/(p1[hadron(-1)]*p1[0]);
	xjac1 = -std::log(eta);
	x1 = eta*std::exp((rng->operator()())*xjac1);

	eta = (p1[-1]*p1[0])/(p1[-1]*p1[hadron(0)]);
	xjac2 = -std::log(eta);
	x2 = eta*std::exp((rng->operator()())*xjac2);

	amp.next_to_leading_order_fini(x1, x1*xjac1, x2, x2*xjac2, 
				       time_rate_rf*weight1, p1);
	
	amp.next_to_leading_order_fini1();
	_M_jet -> userfunc(p1, amp);
	
	p3[hadron(-1)] = p1[hadron(-1)]; 
	p3[hadron( 0)] = p1[hadron( 0)]; 
	
	for(ii = p1.lower(); ii < (int) np; ii++) p3[ii] = p1[ii];
	p3[-1] /= x1; p3[np] = (1.0-x1)*p3[-1];
	amp.next_to_leading_order_finix1();
	_M_jet -> userfunc(p3, amp);
	
	for(ii = p1.lower(); ii < (int) np; ii++) p3[ii] = p1[ii];
	p3[0] /= x2; p3[np] = (1.0-x2)*p3[0];
	amp.next_to_leading_order_finix2();
	_M_jet -> userfunc(p3, amp);
      }
	...
	for(j = i + 1; j <= (int) np; j++)
	  for(k = -1; k <= (int) np; k++)
	    if(k != j && k != i && j > 0 && _M_proc->dipole_index(i, j, k))
