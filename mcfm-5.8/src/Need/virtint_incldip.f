...
c--->>>P.S.
      ...
      logical debug222
      double precision loContrib(0:nSubProcess-1)
      double precision virtContrib(0:nSubProcess-1)
      double precision vcX1overZ(0:nSubProcess-1)
      double precision vcX2overZ(0:nSubProcess-1)
      double precision procReference(0:nSubProcess-1)
      double precision f_X1overZ, f_X2overZ, totalFactor
      double precision cWeightLO, cWeight0, cWeight1, cWeight2
      double precision dWeightLO, dWeight0, dWeight1, dWeight2
c--->>>P.S. end
...
c----->>>P.S. set to zero
      if(creategrid.and.bin)then
         call iniContribution(loContrib)
         call iniContribution(virtContrib)
         call iniContribution(vcX1overZ)
         call iniContribution(vcX2overZ)

         call iniContribution(procReference)
         debug222 = .false.
         f_X1overZ = 0d0
         f_X2overZ = 0d0
      endif
...
c--->>>P.S. collect weights  for Quark---Quark
     ...
     &                +AP(q,q,1)-AP(q,q,3)+Q2(q,q,q,1)-Q2(q,q,q,3)))   !!!     &  *fx1(j)*fx2(k)
     ...
     & + msq(j,g)*(AP(g,q,2)+Q2(g,q,q,2)))                             !!! *fx1(j)*fx2z(k)/z
         ...
         if(debug222.and.((xmsq-tmp).ne.0d0))then
            dWeightLO=cWeightLO*fx1(j)*fx2(k)
            dWeight0 = cWeight0*fx1(j)*fx2(k)
            dWeight1 = cWeight1*fx1z(j)*fx2(k)/z
            dWeight2 = cWeight2*fx1(j)*fx2z(k)/z

            totalFactor = 
     &       dWeightLO + dWeight0 + dWeight1 + dWeight2

            print*,"QQ",j,k
            print*, dWeightLO,dWeight0,dWeight1,dWeight2
            print*,'my = ',totalFactor,' mcfm = ',xmsq-tmp
            print*,'weights: LO = ',cWeightLO,' 0 = ',cWeight0,
     &           ' 1= ',cWeight1,' 2= ',cWeight2
            print*,"------------------------"
          endif
      endif
c--->>>P.S. end
...
C--QbarQbar
     ...
     & + msq(j,g)*(AP(g,a,2)+Q2(g,a,a,2)))*fx1(j)*fx2z(k)/z
c--->>>P.S. collect weights for antiQuark--antiQuark
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &                +AP(a,a,1)-AP(a,a,3)+Q2(a,a,a,1)-Q2(a,a,a,3)))   !!!     & *fx1(j)*fx2(k)
...
c--->>>P.S. collect weights for Quark--antiQuark
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &                +AP(a,a,1)-AP(a,a,3)+Q2(a,a,q,1)-Q2(a,a,q,3)))     !!!     &   *fx1(j)*fx2(k)
...
c--->>>P.S. collect weights for antiQuark--Quark
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &               +AP(q,q,1)-AP(q,q,3)+Q2(q,q,a,1)-Q2(q,q,a,3))) !!!     &  *fx1(j)*fx2(k)
...
c--->>>P.S. collect weights for Glue--Glue
     ...
     &                +AP(g,g,1)-AP(g,g,3)+Q2(g,g,g,1)-Q2(g,g,g,3)))   !!!     &      *fx1(g)*fx2(g)
...
c--->>>P.S. collect weights for Glue--Quark
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &               +AP(q,q,1)-AP(q,q,3)+Q2(q,q,g,1)-Q2(q,q,g,3)))    !!!     &        *fx1(g)*fx2(k)
...
c--->>>P.S. collect weights for Glue--AntiQuark
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &               +AP(a,a,1)-AP(a,a,3)+Q2(a,a,g,1)-Q2(a,a,g,3)))  !!!   &  *fx1(g)*fx2(k)
...
c--->>>P.S. collect weights for Quark-Glue
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     & +msq(j,g)*(zip
     &               +AP(q,q,1)-AP(q,q,3)+Q1(q,q,g,1)-Q1(q,q,g,3)
     &               +AP(g,g,1)-AP(g,g,3)+Q2(g,g,q,1)-Q2(g,g,q,3)))  !!!     &     *fx1(j)*fx2(g)
...
c--->>>P.S. collect weights for AntiQuark-Glue
      if((currentPDF.eq.0).and.creategrid.and.bin)then
     ...
     &               +AP(g,g,1)-AP(g,g,3)+Q2(g,g,a,1)-Q2(g,g,a,3)))!!!     &        *fx1(j)*fx2(g)
...
c----->>>P.S. collect ALL weights
        if(creategrid.and.bin)then
            call collectWeights(j,k,cWeightLO,loContrib)
            call collectWeights(j,k,cWeight0,virtContrib)
            cWeight1 = cWeight1 * f_X1overZ/z
            call collectWeights(j,k,cWeight1,vcX1overZ)
            cWeight2 = cWeight2 * f_X2overZ/z
            call collectWeights(j,k,cWeight2,vcX2overZ)
            
            totalFactor = 
     &          cWeightLO*fx1(j)*fx2(k)  + 
     &           cWeight0*fx1(j)*fx2(k)  + 
     &           cWeight1*fx1z(j)*fx2(k) + 
     &           cWeight2*fx1(j)*fx2z(k)
            
            call collectReference(j,k,totalFactor,procReference)
        endif
c----->>>P.S. end
...
c--->>>P.S. multiply by totalFactor
        if(creategrid.and.bin)then !--->>>P.S. scale with factor
           totalFactor = flux*xjac*pswt/BrnRat
           call scaleContribution(totalFactor,loContrib)
           call scaleContribution(totalFactor,virtContrib)
           call scaleContribution(totalFactor,vcX1overZ)
           call scaleContribution(totalFactor,vcX2overZ)

           call scaleContribution(totalFactor,procReference)
        endif
c--->>>P.S.end
...
c--->>>P.S. multiply by totalFactor
         if(creategrid.and.bin)then     !--->>>P.S. scale with factor
c---------- M.S. for some reason the weights are no longer scaled by 1/itms
            totalFactor = 1d0
            call scaleContribution(totalFactor,loContrib)
...
c--->>>P.S.  filling grid with current weight
        if (creategrid.and.bin) then
            if(debug222)then
             print*," --->> after all factors"
             print*,"x1,x2 = ",virtContrib
             print*," x1/z = ",vcX1overZ
             print*," x2/z = ",vcX2overZ
             print*,""
             print*," ref = ",procReference
             print*,""
             print*, "x1= ",xx(1)," x2 = ",xx(2)," scale = ",facscale**2
             print*,"x1/z = ",x1onz," x2/z = ",x2onz
             print*,"------"
            endif
c------------  QCD coupling removal -------------------------
            totalFactor = 1d0/ason2pi
            call scaleContribution(totalFactor,virtContrib)
            call scaleContribution(totalFactor,vcX1overZ)
            call scaleContribution(totalFactor,vcX2overZ)

C            write(6,*) "sutt virt_incldip 0"

C            do scum = 0,nSubProcess-1
C               write(6,*) loContrib(scum), virtContrib(scum),
C     .                    vcX1overZ(scum), vcX2overZ(scum)  
C            end do

c------------  QCD coupling removal -------------------------
            call fill_grid(xx(1),xx(2),facscale,pjet,loContrib,0)
            call fill_grid(xx(1),xx(2),facscale,pjet,virtContrib,1)
            call fill_grid(x1onz,xx(2),facscale,pjet,vcX1overZ,1)
            call fill_grid(xx(1),x2onz,facscale,pjet,vcX2overZ,1)
            
            call fill_grid_reference(pjet, val, val2)
            call fill_process_reference(pjet,procReference)

            if(debug222)then
               print*,"MCFM value = ",val
               print*," ---------   END       -----------"
            endif
        endif 
