module new_as
  ...
  public :: na_handle
  public :: na_Init, na_Value, na_Del
  public :: na_nfRange, na_nfAtQ, na_QrangeAtNf
...
contains
  ...
  !---------------------------------------------------
  subroutine na_Init(nah, alfas, Q, nloop, fixnf, muMatch_mQuark)
    use qcd; 
    ...
    do i = lbound(quark_masses, dim=1), ubound(quark_masses, dim=1)
       if (tlo > tOfQ(nah%muMatch_mQuark*quark_masses(i))) nah%nlo = i
       if (thi > tOfQ(nah%muMatch_mQuark*quark_masses(i))) nah%nhi = i
    end do
    ...
    do i = nah%nlo, nah%nhi
       seg => nah%seg(i)
       nah%seg(i)%tlo = max(tlo, tOfQ(nah%muMatch_mQuark*quark_masses(i)))
       if (i < nah%nhi) then
          seg%thi = tOfQ(nah%muMatch_mQuark*quark_masses(i+1))
       else
          seg%thi = thi
       end if
       ...
       !-- lazy mans option for getting number of loops automatically
