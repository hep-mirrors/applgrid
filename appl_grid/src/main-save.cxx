...
extern "C" void   dglapinit_(const char* name, const int& set);
...
int main() {
  ...
  int Npdf = 0;
  // setup gavins code
  dglapinit_(_pdfname.c_str(), Npdf);



  appl_pdf* _pdf = appl::appl_pdf::getpdf("nlojet");

  const int Np = _pdf->Nproc();
  ...
  for ( int it=0 ; it<1 ; it++ ) { 
    ...
    for ( int i=0 ; i<N ; i++ ) { 
      ...
      double _alpha = alphaspdf_(Q)/(2*M_PI);
  ...
  //  hgg = g.convolute(pdf, appl::_genpdf_jetrad, _alphas, 1, 0);
  hgg = g.convolute(dglapeval_, alphaspdf_, 1, 0);
