...
c      COMMON/ranno/idum
      ...
      double precision fval,fval2

c  P.S.
      double precision bornWeight(0:nSubProcess-1), totalFactor
...
c --- P.S. initialize array
      if(bin.and.creategrid)then
         call iniContribution(bornWeight)
...
c----   P.S. save weight
        if(bin.and.creategrid)then
c----      print*,j,k,msq(j,k)
           call collectWeights(j,k,msq(j,k),bornWeight)
           call collectReference(j,k,xmsqjk,procReference)
        endif
c----   P.S. end
      endif
...
c----   P.S. multiply by totalFactor
        if(bin.and.creategrid)then ! P.S. scale with factor
            totalFactor = flux*pswt/BrnRat
            call scaleContribution(totalFactor,bornWeight)
...
c-----   M.S. for some reason, the val and val2 values are no
c-----   longer scaled by 1/itmx, so presumably this is done
c-----   elsewhere, so we shouldn't do it for the weights either  
         fval=val/dfloat(itmx)
         fval2=val2/dfloat(itmx)**2

c-----   P.S. multiply by totalFactor
         if(creategrid)then     ! P.S. scale with factor
c-----      M.S. see comment above
...
C          print*," lowint: y34 = ",wrap
C          print*," lowint: p3 = ",pjet(3,4),pjet(3,1),pjet(3,2),pjet(3,3)
C          print*," lowint: p4 = ",pjet(4,4),pjet(4,1),pjet(4,2),pjet(4,3)
            call fill_grid(xx(1),xx(2),facscale,pjet,bornWeight,0)
